...
def test_negative_term_with_whitespace_still_excludes_matching_documents(tmp_path: Path):
    store = FtsStore(index_dir=tmp_path / "tantivy")
    store.upsert(
        [
            {"chunk_id": "c1", "doc_id": "d1", "text": "예산 취소"},
...
def test_negative_phrase_with_whitespace_still_excludes_exact_phrase(tmp_path: Path):
    store = FtsStore(index_dir=tmp_path / "tantivy")
    store.upsert(
        [
            {"chunk_id": "c1", "doc_id": "d1", "text": "예산 품의서 초안 검토"},
